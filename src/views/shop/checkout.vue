          ...
          <div class="box-inner js-checkout-address-panel ">
            <div class="address-common-table js-multiple-address-panel">
              <ul class="address-item-list clear js-address-item-list">
                <li class="js-choose-address  " :class="{'selected-address-item':receiveIndex==index}" @click="chooseReceive(index)" v-for="(receive,index) in receiveInfo">
                  <div class="address-item">
                    <div class="name-section"> {{ receive.name }}  </div>
                    <div class="mobile-section">{{receive.phone }}</div>
                    <div class="detail-section"> {{receive.province }} {{ receive.city }} {{receive.county}}<br>
                      {{ receive.add }} </div>
                  </div>
          ...
          <div class="invoice-detail"> 发票抬头：
            <div class="radio-box">
              <label>
                ...
                <span class="blue-radio " :class="{'blue-radio-on':invoice.personal}" @click="checkedInvoice(true)"><a class="blue-position"></a></span>  个人
              </label>
              <label>
                <input type="radio" class="hide">
                <span class="blue-radio" :class="{'blue-radio-on':!invoice.personal}" @click="checkedInvoice(false)"><a class="blue-position"></a></span>  单位
              </label>
            </div>
      ...
      <div class="gray-box">
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            <p> 商品总计： <span>¥ {{ checkedTotalPrice }}</span> </p>
        ...
        <div class="box-inner">
          <div class="last-payment clear">
            ...
            <span class="prices fn-right">应付金额： <em>¥ {{ checkedTotalPrice + freight }}</em></span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    //已选择的商品信息
    checkedGoods () {
      if (this.$store.getters.checkedGoods.length != 0){
        console.log(this.$store.getters.checkedGoods)
