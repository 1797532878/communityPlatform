    ...
    <div class="content page-order-payment">
      <div class="gray-box clear">
        ...
        <div class="box-inner payment-checkout-panel clear">
	    	            <span class="jianguo-blue-main-btn big-main-btn js-payment-order" @click="payNowHandle(orderInfo.orderId)"
      ...
      <div class="confirm-detail">
        ...
        <p class="info-detail">详细地址：{{orderInfo.receiveInfo.province}} {{orderInfo.receiveInfo.city}}
          {{orderInfo.receiveInfo.country}} {{orderInfo.receiveInfo.add}}</p>
      </div>
      <div class="confirm-detail">
      ...
      <div class="confirm-goods-table">
        <div class="cart-items clear" v-for="(item,index) in orderInfo.goodsData">
          <div class="name fn-left hide-row">
            <div class="name-cell">
              <a href="javascript:;" :title="item.title + '(' + item.spec_json.show_name + ')'" target="_blank">
  ...
  computed: {
    ...
    orderInfo () {
      let orderInfo = this.$store.state.orderData.filter((order) => {
        return order.orderId === Number(this.$route.query.orderId)
      })[0]
      return orderInfo
    }
  },
