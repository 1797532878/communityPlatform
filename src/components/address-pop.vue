                ...
                <div class="address-form">
                  ...
                  <div class="module-form-row">
                    <div class="form-item-v3" :class="{'form-invalid-item':phoneError}">
                      <input type="text" class="js-verify" placeholder="手机号" v-model="receive.phone" @blur="inspectPhone" @focus="setPhone">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row clear">
                    <div class="form-item-v3 area-code-w fn-left ">
                  ...
                  <div class="module-form-row clear">
                    ...
                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="receive.countryId">
                        <option value="0">请选择区县</option>
                        <option :value="country.area_id" v-for="(country,index) in countryList">{{country.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row">
...
export default {
  ...
  watch: {
    'receive.countryId'() {
      this.countryList.forEach((country) => {
        if (country.area_id === this.receive.countryId) {
          // 页面只绑定了ID而没有设置名字
          this.receive.country = country.area_name
          return
        }
      })
    },
  ...
  methods: {
    ...
    // 检查是否都填充了需要的  表单元素
    inspectReceive () {
      if (this.receive.name && !this.phoneError && this.receive.province &&
        this.receive.city && this.receive.country && this.receive.add) {
        this.isRight = true
      }else{
        this.isRight = false
      }
