      ...
      <div class="params-colors">
        <ul class="colors-list">
          <li v-for="(sku,index) in item.sku_info">
            <a href="javascript:;" :title="sku.spec_json.show_name" @click="tableIndex(index)" :class="{'active':index == itemIndex}">
...
<!--        <span class="item-blue-btn" @click="addCarPanelHandle(item.sku_info[itemIndex],$event)">加入购物车 </span>-->
...
export default {
  ...
  methods: {
    ...
    //添加购物商品
    addCarPanelHandle (dat_a) {
      // console.log(event.path[0])
      // this.$store.commit('addCarPanelData',{data,event})
      let data = {info:dat_a,count:1}
